...
export function PushEnrollment() {
  ...
  return <div className="space-y-2"><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={enable} className="rounded-xl border border-neutral-300 px-3 py-2 text-sm font-semibold">Enable push on this device</button>{message && <span role="status" className="text-xs text-neutral-600">{message}</span>}</div>{status && <p className="text-xs text-neutral-500">{status.subscribedDevices} device{status.subscribedDevices === 1 ? "" : "s"} subscribed to your CRM account.</p>}</div>;
}
