...
export default async function ContactsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const filtered = contacts.filter(c => (!q || [c.name, c.email, c.phone].some(v => v?.toLowerCase().includes(q))) && (!type || c.items.some(i => i.kind === type)));
  ...
  return <div className="space-y-5">
    <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#8A6A16]">People</p><h1 className="font-premium mt-1 text-3xl font-semibold">Contacts</h1><p className="mt-2 text-sm text-neutral-600">One list of people, linked to their quote, retirement, and contact inquiries.</p></div>
    <form action="/admin/contacts" className="premium-card grid gap-3 rounded-2xl p-4 sm:grid-cols-[1fr_180px_auto]">
      ...
      <select name="type" defaultValue={type} aria-label="Filter by source" className="h-12 rounded-xl border border-neutral-300 bg-white px-4"><option value="">All sources</option><option value="quote">Quotes</option><option value="retirement">Retirement</option><option value="inquiry">Contact form</option></select>
    ...
    <div className="grid gap-3">{pageItems.map(contact => <article key={contact.key} className="premium-card rounded-2xl p-4 sm:p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-2">{contact.items.map(item => <Link key={`${item.kind}:${item.id}`} href={item.href} className="rounded-full border border-neutral-300 px-3 py-1.5 text-xs font-semibold capitalize hover:border-[#C9A227]">{item.kind} · {item.status.replaceAll("_", " ")}</Link>)}</div>
