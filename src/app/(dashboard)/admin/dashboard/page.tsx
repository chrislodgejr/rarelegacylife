...
export default async function AdminDashboardPage() {
  ...
  const active = items.filter(item => item.kind !== "inquiry" && !["closed", "lost", "not_qualified", "do_not_contact", "spam", "placed", "completed"].includes(item.status));
  const newRecords = active.filter(item => ["new", "assigned"].includes(item.status));
  const recent = items.filter(item => item.kind !== "inquiry").slice(0, 8);
  ...
  return <div className="space-y-7">
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      <Metric label="Contacts" value={groupContacts(items.filter(i => i.kind !== "inquiry")).length} href="/admin/contacts" />
    ...
    <div className="grid gap-5 lg:grid-cols-[1.5fr_1fr]">
      ...
      <section className="premium-card rounded-2xl p-5 sm:p-6"><h2 className="font-premium text-2xl font-semibold">Workspaces</h2><div className="mt-4 grid gap-3">
        ...
        <Workspace href="/admin/contacts?type=inquiry" label="Contact form archive (unreviewed)" count={items.filter(i => i.kind === "inquiry").length} />
