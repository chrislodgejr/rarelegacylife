...
export default async function AdminDashboardPage() {
  ...
  const [allLeads, hotLeads, aLeads, retirementRequests, openTasks, applications, placedPolicies] = await Promise.all([
    ...
    supabase.from("retirement_blueprint_requests").select("*", { count: "exact", head: true }),
  ...
  return (
    <div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-4 xl:grid-cols-8">
        ...
        <Metric label="Retirement" value={retirementRequests.count ?? 0} />
      ...
      <section className="premium-card mt-8 rounded-xl p-6">
        ...
        <div className="mt-4 grid gap-3 text-sm text-neutral-700 md:grid-cols-3">
          ...
          <p className="rounded-md bg-neutral-50 p-4">Both website forms arrive in this shared CRM inbox.</p>
          <p className="rounded-md bg-neutral-50 p-4">Chris and Dan can review quote leads and retirement cases together.</p>
        </div>
      </section>
    </div>
  );
