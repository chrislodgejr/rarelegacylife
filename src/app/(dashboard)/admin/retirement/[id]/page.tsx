...
export default async function RetirementCase({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!data) notFound();
  const { data: tasks } = await createAdminClient().from("retirement_case_tasks").select("id,title,due_at,completed_at").eq("request_id", id).order("created_at", { ascending: false });
  return <div className="space-y-5">
    ...
    <section className="premium-card rounded-2xl p-5 sm:p-7"><h2 className="font-premium text-2xl font-semibold">Follow-up tasks</h2>
    ...
    <RetirementCaseForm request={data} />
  </div>;
}
